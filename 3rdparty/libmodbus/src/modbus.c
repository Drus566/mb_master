...
int modbus_set_log_buf(modbus_t *ctx, char *tx_buf_ptr, char *rx_buf_ptr)
{
    if (ctx == NULL || tx_buf_ptr == NULL || rx_buf_ptr == NULL) {
        errno = EINVAL;
        return -1;
    }
    
    ctx->buffer_rx = rx_buf_ptr;
    ctx->buffer_tx = tx_buf_ptr;
    return 0;
}
